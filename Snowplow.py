...
class Snowplow:
    def __init__(self, parent_screen):
        ...
        self.available_moves = ["DOWN", "UP", "LEFT", "RIGHT"]
        self.last_move = "NONE"
    ...
    def move_snowplow(self, event):
        ...
        if event.key == pygame.K_DOWN and "DOWN" in self.available_moves:
            self.y_coor += HCV.MOVE_Y
            self.grid_y_coor = (self.y_coor + HCV.SNOWPLOW_IMG_OFFSET) // HCV.BLOCK_HEIGHT
            self.last_move = "DOWN"
        if event.key == pygame.K_UP and "UP" in self.available_moves:
            self.y_coor -= HCV.MOVE_Y
            self.grid_y_coor = (self.y_coor + HCV.SNOWPLOW_IMG_OFFSET) // HCV.BLOCK_HEIGHT
            self.last_move = "UP"
        if event.key == pygame.K_LEFT and "LEFT" in self.available_moves:
            self.x_coor -= HCV.MOVE_X
            self.grid_x_coor = (self.x_coor + HCV.SNOWPLOW_IMG_OFFSET) // HCV.BLOCK_WIDTH
            self.last_move = "LEFT"
        if event.key == pygame.K_RIGHT and "RIGHT" in self.available_moves:
            self.x_coor += HCV.MOVE_X
            self.grid_x_coor = (self.x_coor + HCV.SNOWPLOW_IMG_OFFSET) // HCV.BLOCK_WIDTH
            self.last_move = "RIGHT"
        return self.x_coor, self.y_coor, self.grid_x_coor, self.grid_y_coor, original_x_coor, original_y_coor

    def change_direction(self, grid_x, grid_y):
        # Remove the last movement direction from list of available moves, so snowplow doesnt continue into barrier
        if self.last_move in self.available_moves:
            self.available_moves.remove(self.last_move)
        ...
        for i in self.available_moves:
            if i == "DOWN":
                ...
                if collision_detected:
                    self.available_moves.remove("DOWN")
            if i == "UP":
                x = grid_x
                y = grid_y - 1
                collision_detected = self.detect_collision(x, y)
                if collision_detected:
                    self.available_moves.remove("UP")
            if i == "LEFT":
                x = grid_x - 1
                y = grid_y
                collision_detected = self.detect_collision(x, y)
                if collision_detected:
                    self.available_moves.remove("LEFT")
            if i == "Right":
                x = grid_x + 1
                y = grid_y
                collision_detected = self.detect_collision(x, y)
                if collision_detected:
                    self.available_moves.remove("Right")
